...
const ModalProductCategoryUpdate = (
  {
    onAfterUpdate,
    onSubmit
  }: {
  ...
  const [idDataProductCategory, setIdDataProductCategory] = useState<number>(0)
  ...
  const submitProductCategoryCreate: SubmitHandler<FormValues> = (values: {
    name: string
    categorySlug: string
    description?: string
    thumbnail?: string
    ...
    const payload: { id: number; name: string; categorySlug: string; description?: string; image?: string } = {
    ...
    return productCategoryApis.updateCategory(payload).then(() => {
  ...
  return (
    <>
      <Modal footer={null} destroyOnClose maskClosable={false} open={visible} onOk={handleOk} onCancel={onClose}>
        <h3>{i18next.t('category_update')}</h3>
